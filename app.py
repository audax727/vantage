...
from flask import Flask, request, jsonify, session, redirect, url_for, render_template, g, send_file
...
def record_sale(user_id, customer_id, items, amount_paid, channel="in_store", timestamp=None):
    ...
    for it in items:
        ...
        line_total = product["sell_price"] * it["qty"]
        total_amount += line_total
        resolved_items.append((product, it["qty"], product["sell_price"]))
...
def signup_page():
    ...
    cur = db.execute(
        "INSERT INTO users (email, password_hash, auth_provider, shop_name, created_at) VALUES (?,?,?,?,?)",
